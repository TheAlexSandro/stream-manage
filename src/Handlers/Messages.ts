...
export class Messages {
  static handle(ctx: Context, bot: Bot) {
    ...
    if (pola.exec(text)) {
      ctx
        .reply(t("en", "start_message", { name: Helper.getName(ctx) }), {
          parse_mode: "HTML",
        })
        .catch(() => {});
          ...
          Database.get("user", "id", chatID, (err, db_result: UserDB) => {
            ...
            if (getStream)
              return bot.api
                ...
                .catch(() => {
                  ctx
                    .reply(t("en", "stream_avail_message"), {
                      parse_mode: "HTML",
                    })
                    .catch(() => {});
                });
          ...
          Database.get("user", "id", chatID, (err, db_result: UserDB) => {
            ...
            if (!streamId)
              return bot.api
                ...
                .catch(() => {
                  ctx
                    .reply(t("en", "not_initial_message"), {
                      parse_mode: "HTML",
                    })
                    .catch(() => {});
                });
            ...
            if (!getSource) {
              var keybs: any[] = [];
              keybs = [btn.text(t("en", "cancel_button"), `cancel_session`)];
              bot.api
                .editMessageText(
                  chatID,
                  message_result.message_id,
                ...
                .catch(() => {
                  ctx
                    .reply(t("en", "send_name_layer_message"), {
                      parse_mode: "HTML",
                      reply_markup: markup.inlineKeyboard(keybs),
                    })
                    .catch(() => {});
          ...
          Database.get("user", "id", chatID, (err, db_result: UserDB) => {
            ...
            if (!Cache.get(`stream_id_${chatID}`)) {
              ...
              bot.api
                ...
                .catch(() => {
                  ctx
                    .reply(t("en", "stream_init_message", { id: streamId }), {
                      parse_mode: "HTML",
                    })
                    .catch(() => {});
                });
              ...
              if (
                ...
                return bot.api
                  ...
                  .catch(() => {
                    ctx
                      .reply(t("en", "stream_change_proc_message"), {
                        parse_mode: "HTML",
                      })
                      .catch(() => {});
                  });
              if (Cache.get(`stream_${streamId}`)) {
                ...
                bot.api
                  ...
                  .catch(() => {
                    ctx
                      .reply(t("en", "stream_started_message"), {
                        parse_mode: "HTML",
                        reply_markup: markup.inlineKeyboard(keybs),
                      })
                      .catch(() => {});
              ...
              if (!Cache.get(`raw_source_${streamId}`))
                return bot.api
                  ...
                  .catch(() => {
                    ctx
                      .reply(t("en", "source_empty_message"), {
                        parse_mode: "HTML",
                      })
                      .catch(() => {});
                  });
              if (
                ...
                return bot.api
                  ...
                  .catch(() => {
                    ctx
                      .reply(t("en", "source_not_select_message"), {
                        parse_mode: "HTML",
                      })
                      .catch(() => {});
                  });
    ...
    if (getSessionNLayer) {
      if (!ctx.message?.text)
        return ctx.reply(t("en", "stream_text_only")).catch(() => {});
      if (text.length > 50)
        return ctx.reply(t("en", "name_length_ex_message")).catch(() => {});
      var keyb = [];
      keyb[0] = [
        btn.url(
          t("en", "example_button"),
    ...
    if (getSessionLayer) {
      if (ctx.message!.photo)
        return ctx.reply(t("en", "pht_warning_message")).catch(() => {});
      if (!ctx.message?.video && !ctx.message?.text && !ctx.message?.document)
        return ctx.reply(t("en", "source_err_message")).catch(() => {});
      ...
      if (
        ...
        return ctx.reply(t("en", "size_exceed_message")).catch(() => {});
      ...
      if (!isUrl) {
        return ctx.reply(t("en", "url_invalid")).catch(() => {});
      }
      ...
      if (text.includes("youtube.com") || text.includes("youtu.be")) {
        if (!Helper.isYT(text)) {
          return ctx.reply(t("en", "yt_inv_message")).catch(() => {});
        }
    ...
    if (getSessionAdd) {
      ...
      if (!/^rtmps:\/\/dc[1-9]\d*-[1-9]\d*\.rtmp\.t\.me\/s\/$/i.exec(text))
        return ctx.reply(t("en", "stream_unsupport")).catch(() => {});
    ...
    if (getSessionKey) {
      ...
      if (!/^\d{8,10}:[A-Za-z0-9_-]{20,}$/i.exec(text))
        return ctx.reply(t("en", "stream_key_invalid")).catch(() => {});
    ...
    if (getSessionStreamLoop) {
      if (
        ...
        return ctx.reply(t("en", "opt_unv_yt_message")).catch(() => {});
      if (!ctx.message?.text)
        return ctx.reply(t("en", "stream_text_only")).catch(() => {});
      if (!/^([0-9]|10|-1)$/i.test(text))
        return ctx.reply(t("en", "loop_max_message")).catch(() => {});
    ...
    if (getSessionStreamBMB) {
      if (!ctx.message?.text)
        return ctx.reply(t("en", "stream_text_only")).catch(() => {});
      if (!/\d+/i.exec(text))
        return ctx.reply(t("en", "must_numb_message")).catch(() => {});
      if (!/^([2-4][0-9]{3}|5000)$/i.test(text))
        return ctx.reply(t("en", "bmb_max_message")).catch(() => {});
    ...
    if (getSessionStreamSpeed) {
      ...
      if (!/^([1-9]|10)$/i.test(text))
        return ctx.reply(t("en", "speed_max_message")).catch(() => {});
